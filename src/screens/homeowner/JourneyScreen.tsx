...
// Static stage descriptions — matched by stage name from backend
const STAGE_META: Record<string, { description: string; checklist: string[] }> = {
  ...
  'Clear Contingencies (sell)': { description: 'Buyer removes inspection and financing contingencies.', checklist: ['Complete buyer inspection', 'Provide requested repairs', 'Receive contingency removal'] },
};
...
function buildStages(engagement: Engagement): JourneyStage[] {
  return engagement.stages.map(s => {
    ...
    const status: JourneyStage['status'] =
      apiStatus === 'done'    ? 'completed' :
      apiStatus === 'current' ? 'active'    : 'pending';
    return {
      id: s.stageNumber,
      title: s.name,
...
export default function JourneyScreen() {
  ...
  return (
    <View style={styles.screen}>
      ...
      >
        {!activeEng ? (
          <Text style={[Typography.sm, { textAlign: 'center', marginTop: 40 }]}>
            No active {tab} journey found.
          </Text>
        ) : (
          <>
